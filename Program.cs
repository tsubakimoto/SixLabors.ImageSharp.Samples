...
namespace SixLabors.ImageSharp.Samples
{
    static class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var img = Image.Load("sample.png"))
            {
                var font = SystemFonts.CreateFont("Consolas", 10);

                using (var img2 = img.Clone(ctx => ctx.ApplyScalingWaterMark(font, "A short piece of text", Color.HotPink, 5, false)))
                {
                    img2.Save("output/simple-alpha.png");
                    Console.WriteLine("Saved output/simple-alpha.png");
                }

                using (var img2 = img.Clone(ctx => ctx.ApplyScalingWaterMark(font, "みじかいぶんしょう", Color.HotPink, 5, false)))
                {
                    img2.Save("output/simple-kana.png");
                    Console.WriteLine("Saved output/simple-kana.png");
                }
